...
private class ResolutionFacadeWithDebugInfo(
    ...
    override fun analyze(element: KtElement, bodyResolveMode: BodyResolveMode) = tryResolve(
        { ResolvingWhat(listOf(element), bodyResolveMode) }
    ) {
        delegate.analyze(element, bodyResolveMode)
    }

    override fun analyze(elements: Collection<KtElement>, bodyResolveMode: BodyResolveMode): BindingContext = tryResolve(
        { ResolvingWhat(elements, bodyResolveMode) }
    ) {
        delegate.analyze(elements, bodyResolveMode)
    }

    override fun analyzeWithAllCompilerChecks(elements: Collection<KtElement>) = tryResolve(
        { ResolvingWhat(elements) }
    ) {
        delegate.analyzeWithAllCompilerChecks(elements)
    }

    override fun resolveToDescriptor(declaration: KtDeclaration, bodyResolveMode: BodyResolveMode) = tryResolve(
        { ResolvingWhat(listOf(declaration), bodyResolveMode) }
    ) {
    ...
    override fun <T : Any> getFrontendService(serviceClass: Class<T>): T = tryResolve(
        { ResolvingWhat(serviceClass = serviceClass) }
    ) {
        delegate.getFrontendService(serviceClass)
    }

    override fun <T : Any> getIdeService(serviceClass: Class<T>): T = tryResolve(
        { ResolvingWhat(serviceClass = serviceClass) }
    ) {
        delegate.getIdeService(serviceClass)
    }

    override fun <T : Any> getFrontendService(element: PsiElement, serviceClass: Class<T>): T = tryResolve(
        { ResolvingWhat(listOf(element), serviceClass = serviceClass) }
    ) {
        delegate.getFrontendService(element, serviceClass)
    }

    override fun <T : Any> tryGetFrontendService(element: PsiElement, serviceClass: Class<T>): T? = tryResolve(
        { ResolvingWhat(listOf(element), serviceClass = serviceClass) }
    ) {
        delegate.tryGetFrontendService(element, serviceClass)
    }

    override fun <T : Any> getFrontendService(moduleDescriptor: ModuleDescriptor, serviceClass: Class<T>): T = tryResolve(
        { ResolvingWhat(serviceClass = serviceClass, moduleDescriptor = moduleDescriptor) }
    ) {
        delegate.getFrontendService(moduleDescriptor, serviceClass)
    }

    private inline fun <R> tryResolve(resolvingWhat: () -> ResolvingWhat, body: () -> R): R {
        try {
            return body()
        } catch (e: Throwable) {
...
private fun StringBuilder.appendElement(element: PsiElement) {
    ...
    information["isValid"] = element.isValid.toString()
    information["isPhysical"] = element.isPhysical.toString()
    if (element is PsiFile) {
        information["containingFile.name"] = element.containingFile.name
    }
    val moduleInfo = element.getModuleInfo()
    information["moduleInfo"] = moduleInfo.toString()
    information["moduleInfo.platform"] = moduleInfo.platform?.toString()
    information["ideaModule"] = ModuleUtil.findModuleForFile(element.containingFile)?.name
