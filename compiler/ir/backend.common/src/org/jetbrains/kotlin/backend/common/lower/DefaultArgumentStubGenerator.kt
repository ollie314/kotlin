...
private fun IrFunction.generateDefaultsFunctionImpl(context: CommonBackendContext): IrFunction {
    ...
    val newTypeParameters = typeParameters.map {
        ...
        IrTypeParameterImpl(
            ...
            it.endOffset,
            it.origin,
            IrTypeParameterSymbolImpl(typeDescriptor),
            it.name,
            it.index,
            it.isReified,
            it.variance
        ).also { p ->
            typeDescriptor.bind(p)
            p.parent = newFunction
            p.superTypes += it.superTypes
        }
    }

    newFunction.returnType = returnType
    newFunction.dispatchReceiverParameter = dispatchReceiverParameter?.copy(newFunction)
    newFunction.extensionReceiverParameter = extensionReceiverParameter?.copy(newFunction)
